...
long get_divider(long magnitude) {
    var half_magnitude = magnitude / 2 + 1;
    return (long)pow(10, half_magnitude) + 1;
}
...
long sum_invalid_ids_of_same_magnitude(long magnitude, long start = DEFAULT, long end = DEFAULT) {
    ...
    var divider = get_divider(magnitude);
    ...
    return (first.multiplier + last.multiplier) * count * divider / 2L;
}
